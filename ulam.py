...
def prime(number):
    # In fact, we test if number is NOT a prime, cause it's faster by shortcut
    if number % 2 != 0:
        return oddIsPrime(number)
    else:
        return False

# Test primarity of an odd number by testing its
# divisibility by odds between 3 and squre root off this odd number
def oddIsPrime(oddNumber):
    for i in range(3, int(math.sqrt(oddNumber))+1, 2):
        if oddNumber % i == 0:
            return False
    return True
